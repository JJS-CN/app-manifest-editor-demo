...
4. 执行`npm install`安装js运行库

5. 根据项目实际需求和manifest层级情况，修改example-json里的配置内容；执行步骤6时，会在output文件夹生成AndroidManifest.json文件,可查看层级和验证修改是否成功

6. 根据实际需求执行命令`node index.js --object-path element.application.meta-data[0] --set-elements examples/data-attributes.json`
...
11. 执行命令重新生成aab包`java -jar bundletool.jar build-bundle --modules=base.zip --output=app_new.aab`
